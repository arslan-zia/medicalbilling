...
class Login extends DB_connection {
	...
	function listChats() {
		try {
			...
			$select = 'SELECT chats.id, chats.sender_id, chats.receiver_id, chats.last_message, chats.is_read_by_sender, chats.is_read_by_receiver, chats.is_read_by_admin, chats.created_at, sender.user_id AS sender_id, sender.full_name AS sender_name, sender.profile_picture AS sender_picture, receiver.user_id AS receiver_id, receiver.full_name AS receiver_name, receiver.profile_picture AS receiver_picture
			FROM chats 
			...
			if (mysqli_num_rows($chats) > 0) {
				...
				while($chat = mysqli_fetch_object($chats)) {
					...
					$displayName = htmlspecialchars($chat->sender_id == $userId ? $chat->receiver_name : $chat->sender_name);
					...
					if (!$isAdmin && $userId == $chat->sender_id) {
						$isRead = $chat->is_read_by_sender;
					} else if (!$isAdmin && $userId == $chat->receiver_id) {
						$isRead = $chat->is_read_by_receiver;
					} else {
						$isRead = $chat->is_read_by_admin;
					}

					$divClass = $isRead == 1 ? '' : 'new-message';

					$html .= '
						<div class="chat-user ' . $divClass . '" data-sender-id="' . $chat->sender_id . '" data-receiver-id="' . $chat->receiver_id . '" data-user-id="' . $chat->user_id . '" data-user-name="' . htmlspecialchars($displayName) . '" data-chat-id="' . $chat->id . '" onClick="openChat(this)">
    ...
    try {
			...
			$chatQuery = 'SELECT c.id, c.sender_id, c.receiver_id, c.last_message, c.is_read_by_sender, c.is_read_by_receiver, c.is_read_by_admin, c.created_at, s.user_id AS sender_id, s.full_name AS sender_name, s.profile_picture AS sender_picture, r.user_id AS receiver_id, r.full_name AS receiver_name, r.profile_picture AS receiver_picture
			FROM chats c
			...
			if (mysqli_num_rows($chats) > 0) {
				while($chat = mysqli_fetch_object($chats)) {
					$processedChats[] = $chat->sender_id == $userId ? $chat->receiver_id : $chat->sender_id;
			...
			foreach ($userResults as $user) {
				...
				if (!$isAdmin) {
					$html .= '
						<div class="chat-user" data-sender-id="' . $userId . '" data-receiver-id="' . $user->user_id . '" data-user-id="' . $user->user_id . '" data-user-name="' . htmlspecialchars($displayName) . '" data-chat-id="' . $chat->id . '" onClick="openChat(this)">
							<img src="' . $profilePic . '" class="avatar" />
							<div class="chat-info">
								<span class="chat-name">' . $displayName . '</span>
	...
	function sendMessage($post) {
		try {
			...
			$chatQuery = 'SELECT id, sender_id, receiver_id, is_read_by_sender, is_read_by_receiver, is_read_by_admin, is_deleted FROM chats 
			...
			if ($chatResult && $chatResult->num_rows > 0) {
				...
				if ($currentUserId == $chatRow['sender_id']) {
					if ($isAdmin) {
						$updateIsReadStatus = ', is_read_by_receiver = 0, is_read_by_sender = 1, is_read_by_admin = 1';
					} else {
						$updateIsReadStatus = ', is_read_by_sender = 1, is_read_by_receiver = 0, is_read_by_admin = 0';
					}
				} elseif ($currentUserId == $chatRow['receiver_id']) {
					if ($isAdmin) {
						$updateIsReadStatus = ', is_read_by_sender = 0, is_read_by_receiver = 1, is_read_by_admin = 1';
					} else {
						$updateIsReadStatus = ', is_read_by_sender = 0, is_read_by_receiver = 1, is_read_by_admin = 0';
					}
				} else {
					$updateIsReadStatus = ', is_read_by_sender = 0, is_read_by_receiver = 0, is_read_by_admin = 1';
				}
		
				//Update last message
				$updateChat = "UPDATE chats SET last_message = '$message', is_deleted = 0, created_at = '$createdAt' $updateIsReadStatus WHERE id = '$chatId'";
	...
	function markedAsRead($chatId) {
		try {
			...
			//Check if chat already exists
			$select = 'SELECT id, receiver_id, sender_id, is_read_by_sender, is_read_by_receiver, is_read_by_admin
			FROM chats 
			WHERE id = ' . $chatId . '
			AND (' . ($isAdmin ? '1=1' : '(chats.sender_id = ' . $currentUserId . ' OR chats.receiver_id = ' . $currentUserId . ')') . ')';
			...
			$chat = mysqli_fetch_object($chatResult);

			$isRead = 0;
			
			if (!$isAdmin && $currentUserId == $chat->sender_id && $chat->is_read_by_sender == 0) {
				$isRead = 'is_read_by_sender = 1';
			} else if (!$isAdmin && $currentUserId == $chat->receiver_id && $chat->is_read_by_receiver == 0) {
				$isRead = 'is_read_by_receiver = 1';
			} else if ($isAdmin && $chat->is_read_by_admin == 0) {
				$isRead = 'is_read_by_admin = 1';
			}
			...
			//Update last message
			$updateChat = 'UPDATE chats SET ' . $isRead . ' WHERE id = ' . $chatId;
			...
			return [
				'success' => true,
				'message' => 'Chat already marked as read successfully.',
				'data' => [
					'chat_id' => $chatId
				]
			];
	...
	function listAdminChats() {
		try {
			...
			JOIN inv_qne_users receiver 
			ON receiver.user_id = chats.receiver_id
			WHERE chats.sender_id != ' . $userId . ' AND chats.receiver_id != ' . $userId .  '
			ORDER BY chats.created_at DESC
			LIMIT 10';
	...
	function searchAdminUsers($search) {
    try {
			$DB = new DB_connection();
			...
			AND chats.receiver_id != $userId 
			AND (
				sender.full_name LIKE '%$search%'
				OR receiver.full_name LIKE '%$search%'
			)
			ORDER BY chats.created_at DESC
			LIMIT 10";
		...
		try {
			...
			if (mysqli_num_rows($chats) > 0) {
				...
				while($chat = mysqli_fetch_object($chats)) {
					...
					$html .= '
						<li class="list-group-item chat-user '. $divClass .'" data-sender-id="' . $chat->sender_id . '" data-receiver-id="' . $chat->receiver_id . '" data-user-id="' . $chat->user_id . '" data-user-name="' . htmlspecialchars($displayName) . '" data-chat-id="' . $chat->id . '" onClick="openChat(this)">
							<img src="' . $profilePic . '" alt="" class="rounded-circle me-2" width="30" height="30"> ' . htmlspecialchars($displayName) . '
						</li>
					';
				}
    ...
    try {
			...
			if (mysqli_num_rows($chats) > 0) {
				while($chat = mysqli_fetch_object($chats)) {
					...
					$html .= '
						<li class="list-group-item chat-user ' . $divClass . '" data-sender-id="' . $chat->sender_id . '" data-receiver-id="' . $chat->receiver_id . '" data-user-id="' . $chat->user_id . '" data-user-name="' . htmlspecialchars($displayName) . '" data-chat-id="' . $chat->id . '" onClick="openChat(this)">
							<img src="' . $profilePic . '" alt="" class="rounded-circle me-2" width="30" height="30"> 
								' . $displayName . '
						</li>
					';
				}
			...
			foreach ($userResults as $user) {
				...
				if (!$isAdmin) {
					$html .= '
						<li class="list-group-item chat-user" data-sender-id="' . $userId . '" data-receiver-id="' . $user->user_id . '" data-user-id="' . $user->user_id . '" data-user-name="' . htmlspecialchars($displayName) . '" data-chat-id="' . $chat->id . '" onClick="openChat(this)">
							<img src="' . $profilePic . '" alt="" class="rounded-circle me-2" width="30" height="30"> 
								' . $displayName . '
						</li>
					';
					...
					if ($chat->sender_id == $userId && !$chat->is_file) {
						$html .= '
							<div class="message-container text-start">
								<div class="chat-message self">' . $chat->message . '</div>
								<div class="message-timestamp">' . date('D F Y - h:i A', strtotime($chat->created_at)) . '</div>
							</div>
						';
					} else if ($chat->sender_id != $userId && !$chat->is_file) {
						$html .= '
							<div class="message-container text-end">
								<div class="chat-message other">' . $chat->message . '</div>
								<div class="message-timestamp">' . date('D F Y - h:i A', strtotime($chat->created_at)) . '</div>
							</div>
						';
					} else if ($chat->sender_id == $userId && $chat->is_file) {
						$html .= '
							<div class="text-start mb-1">
								<span class="badge bg-secondary">📎 <a href="' . str_replace('assets/', '', ASSETS) . 'uploads/chat_files/' . $chat->message . '" target="_blank">' . $chat->message . '</a></span>
								<div class="message-timestamp">' . date('D F Y - h:i A', strtotime($chat->created_at)) . '</div>
							</div>
						';
					} else if ($chat->sender_id != $userId && $chat->is_file) {
			...
			if ($post['message']) {
				$html = '
					<div class="message-container text-start">
						...
						<div class="message-timestamp">' . date('D F Y - h:i A', strtotime($createdAt)) . '</div>
					</div>
				';
			} else {
				$html = '
					<div class="text-start mb-1">
						<span class="badge bg-secondary">📎 <a href="' . str_replace('assets/', '', ASSETS) . 'uploads/chat_files/' . $chat->message . '" target="_blank">' . $message . '</a></span>
						<div class="message-timestamp">' . date('D F Y - h:i A', strtotime($createdAt)) . '</div>
					</div>
				';
			}
	...
	function lastChatMessageV2($chatId) {
		try {
			$DB = new DB_connection();
			...
			if ($chat) {
				...
				if (!$chat->is_file) {
					$html = '
						<div class="message-container ' . ($isSelf ? 'text-start' : 'text-end') . '">
							<div class="chat-message ' . ($isSelf ? 'self' : 'other') . '">' . $chat->message . '</div>
							<div class="message-timestamp">' . date('D F Y - h:i A', strtotime($chat->created_at)) . '</div>
						</div>
					';
				} else {
					$html = '
						<div class="' . ($isSelf ? 'text-start' : 'text-end') . '">
							<span class="badge bg-secondary">📎 <a href="' . str_replace('assets/', '', ASSETS) . 'uploads/chat_files/' . $chat->message . '" target="_blank">' . $chat->message . '</a></span>
							<div class="message-timestamp">' . date('D F Y - h:i A', strtotime($chat->created_at)) . '</div>
						</div>
					';
		...
		try {
			...
			$html = '
				<div class="message-container text-center">
					<div class="chat-message delete">' . $message . '</div>
					<div class="message-timestamp">' . date('D F Y - h:i A', strtotime($createdAt)) . '</div>
				</div>
			';
	...
	function markedAsReadV2($chatId) {
		try {
			$DB = new DB_connection();
