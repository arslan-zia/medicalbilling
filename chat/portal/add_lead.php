<?php include(dirname(dirname(__FILE__)) . '/class/class.php');
	...
	if(isset($_POST['action']) && $_POST['action'] == 'yes')
	{
		if(isset($_POST['add_lead']) && $_POST['add_lead'] == 'true')
		{
			...
			if($result = $leadModel->addLead($_POST))
			{
				header("Location: " . PORTAL_URL . "view_lead.php?lead_id=".$result);
				exit();	
			}
			else
			{
				header("Location: " . PORTAL_URL . "add_lead.php?action=fail&message=fail to add");
				exit();
			}
		}	
	}
