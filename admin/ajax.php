<?php
	...
	switch($Work)
	{
		...
		case 'deleteChat':
			// $deleteChat = $loginModel->deleteChat($_POST);
		...
		case 'markedAsRead':
			...
			$markedAsRead = $loginModel->markedAsRead($chatId);

			echo json_encode($markedAsRead);
		break;

		case 'listAdminChats':
			$listAdminChats = $loginModel->listAdminChats();
			
			echo $listAdminChats['data'];
		break;

		case 'searchAdminUsers':
			$search = $_GET['search'];
			
			$users = $loginModel->searchAdminUsers($search);
			
			echo $users['data'];
		break;

		case 'listChatsV2':
			$listChatsV2 = $loginModel->listChatsV2();
			
			echo $listChatsV2['data'];
		break;

		case 'searchUsersV2':
			$search = $_GET['search'];
			
			$searchUsersV2 = $loginModel->searchUsersV2($search);
			
			echo $searchUsersV2['data'];
		break;

		case 'singleChatV2':
			$chatId = $_REQUEST['chat_id'];
			
			$singleChatV2 = $loginModel->singleChatV2($chatId);
			
			echo json_encode($singleChatV2['data']);
		break;

		case 'sendMessageV2':
			$sendMessageV2 = $loginModel->sendMessageV2($_POST);
			
			echo json_encode($sendMessageV2);
		break;

		case 'lastChatMessageV2':
			$chatId = $_REQUEST['chat_id'];
			
			$lastChatMessageV2 = $loginModel->lastChatMessageV2($chatId);

			echo json_encode($lastChatMessageV2['data']);
		break;

		case 'deleteChatV2':
			// $deleteChatV2 = $loginModel->deleteChatV2($_POST);

			// echo json_encode($deleteChatV2);
		break;

		case 'markedAsReadV2':
			$chatId = $_REQUEST['chat_id'];
			
			$markedAsReadV2 = $loginModel->markedAsReadV2($chatId);

			echo json_encode($markedAsReadV2);
		break;
