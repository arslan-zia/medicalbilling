<?php include(dirname(dirname(__FILE__)) . '/class/class.php'); 
	...
	if(isset($_POST['action']) && $_POST['action'] == 'yes')
	{
		if(isset($_POST['add_user']) && $_POST['add_user'] == 'true')
		{
			...
			{
				header("Location: " . ADMIN_URL . "view_user.php?user_id=".$result['data']['user_id']);
			}			
		}	
	}
?>
													...
													<form action="" method="post" id="kt_invoice_form" name="add_receive_products" class="form-horizontal" enctype="multipart/form-data">
														...
														<div class="mb-0">
															...
															<div class="row gx-10 mb-5">
																...
																<div class="col-lg-6" id="manager-select" style="display:none;">
																	<!--begin::Input group-->
																	<label class="form-label fs-6 fw-bold text-gray-700 mb-3">Select User's Manager</label>
																	<div class="mb-5">
																		<select class="form-select form-select-solid" data-control="select2" data-hide-search="true" data-placeholder="Select Manager" name="manager_id">
																			...
																			<?php
																				$managers = $loginModel->getManagers();
																				foreach($managers as $manager)
																				{
																					echo '<option value="' . $manager['user_id'] . '">' . $manager['full_name'] . '</option>';
																				}
																			?>	
																		</select>
																	</div>
